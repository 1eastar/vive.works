...
SOP가 필요한 이유를 다시 정리하자면, 브라우저에는 storage, cookie 등의 형태로 유저에 관한 중요한 데이터를 저장하고 있기 때문이고, 이런 정보가 외부로 유출되거나 조작될 수 있다면 보안상 큰 문제가 되기 때문이다. SOP는 외부로부터의 공격을 일부 막아주는 역할을 한다.

* CORS

CORS(Cross Origin Resource sharing) 는 SOP가 있기에 존재하는 정책이다. 개발을 해 본 사람이라면 알 수 있듯, 서비스를 개발함에 있어 다른 Origin과의 요청/응답을 주고받는 건 거의 필수적이다. 그렇기에 **SOP를 어기더라도 CORS를 지키면 괜찮아!** 라고 말하는 예외 조항에 해당하는 정책이 생겨난 거다.

# 요청이 Same Origin 인지는 누가 판단하지?
크롬과 같은 브라우저가 서버로 요청을 보내기 전에 체크한다. 브라우저 내부적으로 구현된 로직에 의해 서버로의 요청을 보내기 이전에 Origin과 target을 먼저 체크한다. 여기서 한가지 궁금했던 점은, 이걸 꼭 브라우저에서 해야하나? 였다. 생각해보면 굳이 브라우저가 아니더라도 요청을 받는 서버 측에서도 처리할 수 있을 것 같았다. 하지만 단순히 생각해보았을 때, 우선 모든 외부로부터의 요청들이 서버로 도달하게 되면 서버 부하가 너무 심해진다. 최악의 경우에는 악의적인 요청에 대한 검증만 하다가 원래 필요한 기능을 제대로 못 할 지도 모른다. 그래서 서버는 이런 보안 관리에 리소스를 낭비하지 않고, 검증된 요청만 받아들여 올바른 응답 데이터를 넘겨주는데 집중할 수 있게끔 하기 위해 브라우저에서 필터링을 해서 요청을 보내는 것이다.
...
여담이지만, 브라우저가 검증을 위해 전송하는 preflight request는 OPTIONS method를 사용한다. 그리고 OPTIONS method를 통한 통신은 요청하는 서버에 아무런 영향을 끼치지 않기에 안전하다고 알려져 있다. OPTIONS method의 주요 목적은 서버에서 어떤 HTTP method를 허용하는지, 어떤 헤더들을 허용하는지 등에 대한 정보를 얻는 것이다.
...
본인이 백엔드 개발자라면 사용하는 프레임워크에 맞는 CORS 세팅을 해주면 된다. 프레임워크 별로 그 방식이 상이하고 그 방법에 대한 내용은 조금만 검색해도 알 수 있으니 여기서 전부 다루지는 않겠다.
...
### webpack dev server
Webpack dev server에서도 비록 개발 단계에서만 사용 가능하지만 프록시 기능(정확하게는 리버스 프록시인데 여기서는 다루지 않겠다.)을 제공한다.
```js
// webpack.config.js
module.exports = {
  devServer: {
...
```
위와 같은 설정을 추가했을 때 흐름을 살펴보자. 웹 서버를 로컬(http://localhost:3000)에 띄워둔 상태에서 브라우저를 통해 'http://localhost:3000/api'에 요청을 보내면 요청을 보낸 곳을 'http://localhost:3000'에서 'https://api.vive.com'으로 수정해준다. 결국 브라우저 입장에서는 'localhost:3000'에서 'localhost:3000'로 요청을 보낸 셈이고, 실제 서버(api.vive.com) 입장에서도 'api.vive.com'에서 온 요청으로 인식하게 되므로 문제가 없다.

## HTML tag 에서의 요청
필요에 따라, 상황에 따라 XHR나 fetch 같은 Browser API를 직접 사용하지 않는 방법이 있다. 가령 HTML 내부의 form, img, script 등의 태그를 통해 요청을 보내는 것이다. 브라우저는 이 요청들을 보낼 때는 Origin 헤더 없이 요청을 날린다. SOP를 지키지 않아도 상관없는 요청들이라는 뜻으로 보인다. 
회사에서 NICE 모듈을 붙일 때 거듭 고민한 결과 이 방식을 이용하기로 했다. 
> NICE는 'https://nice.checkplus.co.kr/~~~' 로 함께 요청을 보내면 모듈 화면에 해당하는 html 파일을 내려주고 있었고 NICE 서버 단에서는 whitelist 추가 같은 기능이 없었다. 즉, NICE 모듈을 이용하고자 하는 클라이언트 단에서 알아서 CORS를 처리를 해야했다.
```js
...
이제는 CORS를 보면 그냥 반갑다. 어떤 게 문제이고 어떻게 해결해야하는지 정확히 알다보니 자신감이 생긴 듯하다. NICE 모듈 작업을 할 당시에는 삽질을 좀 했지만 끝난 후에 돌이켜보면 오히려 날 자극시켜준 NICE에게 감사(?)하다.
