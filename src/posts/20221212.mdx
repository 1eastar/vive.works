...
![alt image](../statics/images/gatsby-icon.png)
...
// codeblock 따로 만들거면 새로 만들어야 함.
// image 컴포넌트는 클릭 시 이미지 확대, 다시 클릭 or ESC 누를 시 원상 복귀. 뒷배경은 살짝 blur.
// algoli-queries.ts 에서 export default는 에러가 뜨고 commonjs 방식을 이용해서 오류가 안 뜸. Tags index.tx에는 잘 되는데 무슨 차이?
