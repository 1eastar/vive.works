...
이게 보기보다 큰 문제인게, 크롤러에서 홈 화면을 404 페이지로 인식해버린다. 즉, `<header>`에 들어있는 title, description, og 등 메타 데이터들이 404 페이지의 데이터로 잡힌다는 뜻이다. 비슷한 이슈가 있을까하고 찾아봤지만 없었다. 몇 년 전부터 [이슈](https://github.com/gatsbyjs/gatsby/issues/5329)로 등록된 내용이 있긴 한데 추천을 가장 많이 받은 답변도 근본적인 해결책이 아니었다.
...
이런저런 시도를 해보다가 결국 `pages/`에서도 `Head`를 사용하지 않고 `react-helmet`을 사용하기로 했다. 사실 처음부터 이렇게 하는 게 코드 측면에서나 정신건강 측면에서나 훨씬 깔끔하게 해결이 가능했다. 하지만 더 좋은 방법이 있지 않을까.. 분명 있을텐데.. 하는 마음으로 꼬박 이틀동안 github issue/discussion 정주행을 하고 여러가지를 시도를 해보면서 알게 된 것도 배운 것도 많기에 꽤 재미있었다.
